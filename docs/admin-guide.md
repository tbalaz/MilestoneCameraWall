...
| `LogMaxMegabytesPerDay` | int | `50` | Per-day log size cap in megabytes, enforced while writing: when a day's file reaches the cap, one final `Daily log cap reached` line is written and further lines that day are dropped and counted (a notice names the count on the next successful write). Bounds a runaway error storm on an unattended kiosk between restarts. `0` disables the cap. |
...
| `-RestartHung` | Switch. Escalates the watchdog from report-only to kill+relaunch when `--health-probe` reports a hung UI thread (exit 2), or an absent `health.json` from a long-running process (exit 3, only while `Health.Enabled` is true — read live from `camerawall.json` on every check, no reinstall needed after editing it). Two protections against kill/restart storms: the probe needs two consecutive runs to confirm a hang, and the watchdog rate-limits itself to at most one hung-kill per 10 minutes (marker under `%LOCALAPPDATA%\GridLookout`; see "Wall-health monitoring" above). Off by default: a hung process is reported, never killed. |
...
- Bootstrap ordering: the logger is created at the `Info` floor **before** `camerawall.json` is
  ...
  succeeds.
- **Retention, plus a per-day size cap.** At startup (right after `camerawall.json` is loaded),
  ...
  pruning and keep every day-file forever. Within a single day, `LogMaxMegabytesPerDay`
  (default `50`) caps how much is written: reaching the cap writes one final `Daily log cap
  reached` line and drops further lines that day (a notice names the dropped count on the next
  successful write). Set it to `0` to disable the cap and keep the old unbounded-per-day behavior.
...
(`RecorderSelectionIncomplete`), whether an explicitly configured `LayoutRecorder` currently
matches no selected recorder (`LayoutCarrierPinned` — see "Multi-recorder walls" below; the wall
keeps its last-known-good layout while this is true), and a count of UI-thread exceptions the
process has caught and logged since start (any nonzero count surfaces as `Degraded` — see the
health status matrix below). It never contains camera
...
never-framed tile, an incomplete recorder selection, a pinned-missing layout carrier
(`LayoutCarrierPinned`), or a nonzero UI-thread exception count (the process caught and logged
an unhandled UI-thread error — sticky until restart) is Degraded; anything else is Healthy.
...
|---|---|
| 0 | Healthy |
| 1 | Degraded (UI responsive, but 1+ tile stalled/never-framed/unavailable, a configured recorder is missing, or the process has swallowed a UI-thread exception) |
| 2 | Unhealthy / hung — CONFIRMED across probe runs: a stale UI pulse alone now reads as **1 (Degraded, suspected)** on the first probe and only becomes exit 2 when a LATER probe finds the same unmoved pulse at least `Health.StaleAfterSeconds` after suspicion was first raised (one sample can't distinguish a hang from a slow Management Server blocking the refresh tick). Exit 2 also still covers the responsive-but-`Running`-with-zero-wall-windows case, which needs no confirmation. While the controller is `Starting`/`Connecting`, the stale threshold is 3x `Health.StaleAfterSeconds` instead of 1x, so a merely-slow connection attempt isn't misjudged as a hang |
| 3 | Absent (no health.json, or recorded process not actually running) |

A pulse that ADVANCED between probe runs — however slowly — resets the suspicion: a slow pump
...
never killed by this script. Two protections keep a merely-slow server from becoming a
kill/restart storm: the probe itself needs TWO consecutive runs to confirm a hang (see the exit
code 2 row above), and the watchdog additionally rate-limits itself to **at most one hung-kill
per 10 minutes** (tracked in `%LOCALAPPDATA%\GridLookout\watchdog-last-hung-kill.txt`; an
unreadable marker only ever means one kill sooner than the cap, and a skipped kill is simply
re-evaluated on the next 1-minute tick). Off by default, since a hung-but-alive process is a
different failure mode than a crashed one and some deployments may want to investigate before an
...
and since the m5 fix (2026-08-21), exit 0 **guarantees** every listed file was written by THIS
request (see the freshness rule below). Fetch them however suits the remote session
(`Copy-Item`/`scp` over the admin share, etc.).
...
| Exit code | Meaning |
|---|---|
| 0 | The running wall answered and finished its capture attempt, and at least one file freshly written by THIS request was found. All listed paths are from this request. |
...
| 4 | The wall answered but NO file from THIS request exists in any checked location — the capture almost certainly failed on the wall's side (locked/idle session, unwritable output directory). The tool prints guidance naming where to look instead of reporting months-old imagery as success. |
...
**Three caveats that matter when reading the result:**

- **Capture failure is exit 4, not silence.** A capture failure on the wall's side (a display
  driver refusing the screen copy, the output directory unexpectedly unwritable) is logged as a
  Warning in the wall's own log and does not crash it — the wall still signals completion. Since
  the m5 fix (2026-08-21), that outcome is its own exit code: **4**, with a printed pointer to
  check the wall's log for `Screenshot capture failed`. (Before that fix, the tool silently
  exited 0 — or worse, printed stale files from an earlier request as if they were the answer.)
- **Exit 0 guarantees freshness from THIS request.** The tool only reports a location whose newest
  `screen-*.png` was written at/after the moment it sent the request (with a 2-second tolerance
  for filesystem timestamp granularity). If no checked location qualifies — the capture wrote
  nothing new anywhere — you get exit 4, never an old image dressed up as today's answer.
- **The interactive session must be unlocked and rendering.** A locked workstation, a disconnected
  ...
  requester exits 4 with guidance instead of files). Either way it is inherent to Windows screen
  capture on a non-rendering desktop, not a GridLookout limitation: check the kiosk console
  session's state before suspecting the video wall itself.
