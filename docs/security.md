...
- **`health.json` location and content.** Lives under the same writable state directory as
  ...
  wall keeps its last-known-good layout while true), a count of UI-thread exceptions the process
  has caught and logged since start (a local stability counter — surfaces as Degraded in the
  probe's verdict; carries no identity), and, per configured
