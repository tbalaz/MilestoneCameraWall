...
public class HealthProbeEvaluatorTests
{
    ...
    [Fact]
    public void Evaluate_UiPulseOlderThanStaleAfterSeconds_UnhealthyOrHung()
    {
    ...
    [Fact]
    public void Evaluate_UiPulseExactlyAtStaleAfterSeconds_TreatedAsStale()
    {
        // Age must be STRICTLY less than the threshold to count as fresh — an exact-boundary pulse
        // is judged stale, matching HealthStatusCalculator's own "< " comparison.
    ...
    [InlineData(ControllerState.Starting)]
    [InlineData(ControllerState.Connecting)]
    public void Evaluate_StartingOrConnecting_PulseStaleBeyondGraceMultiplier_Hung(ControllerState controllerState)
    {
    ...
    [Fact]
    public void Evaluate_Recovering_NoGraceMultiplier_StaleAt31Seconds_Hung()
    {
        ...
        // tight 1x threshold rather than the Starting/Connecting grace.
