...
public static class HealthProbe
{
    ...
    /// <summary>M4 fix: the cross-run hung-suspicion marker — see <see cref="HungStreakMarker"/>.</summary>
    public const string HungStreakFileName = "probe-hung-streak.json";
    ...
    public static int Run(IStateDirectory stateDirectory, string baseDir, WallConfig config, WallConfigLoader loader, TextWriter stdout, FileLogger? logger = null)
    {
        ...
        // M4 fix (2026-08-21 external audit): the hung-suspicion streak marker rides between probe
        // RUNS (each is its own short-lived process) via a small state-dir file — read before the
        // verdict, then persisted/cleared per ProbeVerdict.NewStreak (null = no suspicion). Every
        // failure mode degrades to "no prior streak" (worst case: one extra confirming run before
        // a hung verdict), never to a crash of the probe itself.
        HungStreakMarker? priorStreak = null;
        try
        {
            var streakRaw = store.Read(HungStreakFileName);
            if (streakRaw is not null)
            {
                priorStreak = JsonSerializer.Deserialize<HungStreakMarker>(streakRaw, HealthJsonOptions.Default);
            }
        }
        catch (Exception ex)
        {
            logger?.Warning($"--health-probe: hung-streak marker unreadable ({ex.GetType().Name}) — starting a fresh streak.");
        }

        var verdict = HealthProbeEvaluator.Evaluate(state, pidMatches, DateTime.UtcNow, config.Health.StaleAfterSeconds, priorStreak);

        try
        {
            if (verdict.NewStreak is not null)
            {
                store.Write(HungStreakFileName, JsonSerializer.Serialize(verdict.NewStreak, HealthJsonOptions.Default));
            }
            else
            {
                store.TryDelete(HungStreakFileName);
            }
        }
        catch (Exception ex)
        {
            logger?.Warning($"--health-probe: could not persist the hung-streak marker ({ex.GetType().Name}: {ex.Message}) — hysteresis may take one extra run.");
        }
