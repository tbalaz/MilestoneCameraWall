...
public class ScreenshotRequesterTests : IDisposable
{
    ...
    [Fact]
    public void Run_Success_FallsBackToStaleCandidate_WhenNoCandidateHasAFreshFile()
    {
        // If NOTHING qualifies as fresh (e.g. an internal capture failure that leaves only an old
        // file in place — see ScreenshotResponder's own capture-failure-still-signals-Done contract),
        // still report SOMETHING rather than nothing: this is the one path where exit 0 does not
        // guarantee freshness — see ScreenshotRequester's own "FRESHNESS" doc section.
        ...
        Assert.Equal(ScreenshotRequester.ExitCodeSuccess, exitCode);
        Assert.Contains(staleFile, stdout.ToString());
    }

    [Fact]
    public void Run_NoFilesInAnyCandidate_StillReturnsExitCode0_WithNothingPrinted()
    {
        // A legitimate outcome per ScreenshotRequester's own "WHAT 'SUCCESS' ACTUALLY MEANS" doc
        // section: Done fired (the capture attempt finished) but the capture itself failed
        // internally and wrote nothing — the requester cannot distinguish that from "captured zero
        // screens" and must not report it as a requester-side error.
        var requestName = UniqueEventName("request");
        var doneName = UniqueEventName("done");
