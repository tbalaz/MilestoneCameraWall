...
/// kiosk account (docs/camerawall-security.md, "Writable-state fallback") is exactly the kind of
...
public sealed class ScreenshotResponder : IDisposable
{
        ...
        Action<string>? captureAction = null)
    {
        ...
        {
            // Nit fix (2026-08-21 external audit): the second event's constructor throwing
            // (privilege/ACL surprise between the two calls) used to leak the already-created
            // Request handle for the process's whole life — Program.cs catches the ctor exception
            // and carries on without a responder, so nothing else would ever have disposed it.
