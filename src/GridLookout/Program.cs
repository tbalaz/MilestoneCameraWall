...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        catch (Exception ex)
        {
            ...
            ShowConfigLoadFailedCard(DescribeLogDirectory(logger));
            return;
        }
        ...
        if (string.IsNullOrWhiteSpace(config.ManagementServerUri))
        {
            ...
            ShowConfigMissingCard(loader.EffectiveConfigPath ?? Path.Combine(baseDir, "camerawall.json"));
            return;
        }
        ...
        if (config.Health.Enabled)
        {
            ...
            void WriteHealthState()
            {
                try
                {
                    ...
                    // the file's AGE.
                    var overallStatus = HealthStatusCalculator.Compute(uiPulseFresh: true, controllerState, forms, recorderSelectionIncomplete || layoutCarrierPinned);
    ...
    private static void ShowConfigMissingCard(string configPath)
    {
        ...
        form.ShowStatus(
            ...
            $"Edit {configPath}\nand set ManagementServerUri, then restart.",
            isError: true);
    ...
    /// around loader.LoadOrCreate in Main. Generic text only, no exception detail (kiosk screens
    /// are public); the caller already logged the detail via logger.Error before showing this.</summary>
    /// <param name="logDirectoryDescription">T8(d)/R10: the real log location (see
    /// <see cref="DescribeLogDirectory"/>) — the logger itself may not have finished construction
    /// far enough to be trusted here, so the caller passes the already-computed description.</param>
    private static void ShowConfigLoadFailedCard(string logDirectoryDescription)
    {
        ...
        form.ShowStatus(
            ...
            $"See the log at {logDirectoryDescription} for details, then restart.",
            isError: true);
        Application.Run(form);
    }
