...
public static class SpanGrid
{
    ...
    /// diagnostic — matches the legacy grammar's own row-letter convention. Nit fix (2026-08-21
    /// external audit): past row 26 the plain <c>'A' + index</c> arithmetic emitted punctuation
    /// (<c>'['</c>, <c>'\'</c>, …) into diagnostics — reachable, since a grid may be up to 64 rows.
    /// Excel-style double letters now (…, "Z", "AA", "AB", …), matching how an operator would
    /// naturally name a 27th row anyway.</summary>
    private static string RowLetter(int rowIndex)
    {
