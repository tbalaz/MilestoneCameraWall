...
public sealed class DescriptionPollWorker
{
    ...
    // m6 fix (2026-08-21 external audit): consecutive failed/refused polls since the last success.
    // MilestoneSession.TryGetRecorderDescriptions logs its individual failures at Debug (right for
    // a single blip), but a PERSISTENT failure means Management-Client $layout{} edits silently
    // stop applying — a feature-stopping condition that was invisible at the default Info level.
    // One Warning after the threshold, one Info on recovery, nothing per-tick in between.
    private int _consecutiveFailures;
    private bool _persistentFailureWarned;

    /// <summary>Consecutive failed polls after which ONE Warning names the consequence — ~5
    /// ConfigRefreshSeconds intervals, long enough that a token refresh or transient blip has had
    /// every chance to self-heal first.</summary>
    private const int PersistentFailureWarnThreshold = 5;
    ...
    public void TriggerPollIfIdle()
    {
        ...
        Task.Run(() =>
        {
            try
            {
                ...
                {
                    _latest = result;
                    if (_persistentFailureWarned)
                    {
                        _logger?.Info("Background description poll recovered — Management Client layout edits are being picked up again.");
                    }
                    _consecutiveFailures = 0;
                    _persistentFailureWarned = false;
                }
                else if (result is null && !_shutdown)
                {
                    _consecutiveFailures++;
                    if (!_persistentFailureWarned && _consecutiveFailures >= PersistentFailureWarnThreshold)
                    {
                        _persistentFailureWarned = true;
                        _logger?.Warning(
                            $"Background description poll has failed {_consecutiveFailures} times in a row — Management Client " +
                            "$layout{} edits are NOT currently being picked up (the wall keeps using the last successfully-fetched " +
                            "layout). Individual attempt details are logged at Debug level; check Management Server reachability " +
                            "and this session's REST access.");
                    }
                }
            }
            catch (Exception ex)
