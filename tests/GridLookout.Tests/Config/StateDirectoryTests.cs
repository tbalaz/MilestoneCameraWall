...
public class StateDirectoryTests : IDisposable
{
    ...
    [Fact]
    public void Resolve_ExeDirAndProgramDataOverrideBothUnwritable_ReportsFalse_NamesTheProgramDataPath_DoesNotThrow()
    {
        ...
        // deterministically.
        //
        // m7 fix (2026-08-21 external audit): the pre-fix catch returned TRUE with
        // stateDir = exeDir — claiming the exe dir writable one branch after CanWrite() proved it
        // isn't. The contract now stays truthful: false, with stateDir still naming the ProgramData
        // fallback so subsequent writes fail loudly at the path an admin would actually need to fix.
        var fakeExeDir = Path.Combine(_dir, "not-actually-a-directory.tmp");
        File.WriteAllText(fakeExeDir, string.Empty);
        ...
        Assert.Null(exception);
        Assert.False(writable);
        Assert.Equal(unreachableProgramDataOverride, stateDir);
    }
}
