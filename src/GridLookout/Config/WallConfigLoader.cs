...
public sealed class WallConfigLoader
{
    ...
    public WallConfig LoadOrCreate(string directory, string primaryFileName = "camerawall.json", string? overlayFileName = "camerawall.local.json", string templateFileName = "camerawall.template.json")
    {
        ...
        JsonObject? primaryObject = File.Exists(primaryPath) ? ParseObject(File.ReadAllText(primaryPath), primaryPath) : null;
        JsonObject? overlayObject = (overlayPath is not null && File.Exists(overlayPath)) ? ParseObject(File.ReadAllText(overlayPath), overlayPath) : null;
        ...
        if (!exeDirWritable)
        {
            ...
            statePrimaryObject = File.Exists(statePrimaryPath) ? ParseObject(File.ReadAllText(statePrimaryPath), statePrimaryPath) : null;
    ...
    private static bool TryFindJsonStringValue(string text, string propertyName, out int valueStart, out int valueEnd, out bool hasEscape)
    {
        ...
        // m3 fix (2026-08-21 external audit): the pre-fix single Regex.Match anchored on the FIRST
        // occurrence of the key ANYWHERE in the raw text — comment-blind, so a commented-out
        // `// "Password": "old"` line earlier in the file won the anchor, the splice then patched
        // the COMMENT, and the real plaintext below survived on disk while the migration reported
        // success on every boot. Now every occurrence is considered in order and any candidate
        // sitting on a `//`-commented line (the one comment style the shipped template uses; a
        // /* */ block would fail the structural checks and fall back to the comment-stripping
        // reserialize path, which parses properly) is skipped; the first UNCOMMENTED structural
        // match wins.
        foreach (Match keyMatch in Regex.Matches(text, "\"" + Regex.Escape(propertyName) + "\"", RegexOptions.IgnoreCase))
        {
            int lineStart = text.LastIndexOf('\n', Math.Max(0, keyMatch.Index - 1)) + 1;
            if (text.IndexOf("//", lineStart, keyMatch.Index - lineStart, StringComparison.Ordinal) >= 0)
            {
                continue; // this occurrence is commented out — keep looking
            }
            ...
            {
                continue; // not a key:value occurrence (e.g. the word inside some other string)
            }
            i++;
            while (i < text.Length && char.IsWhiteSpace(text[i]))
            {
            ...
            if (i >= text.Length || text[i] != '"')
            {
                continue; // not a string value at this occurrence
            }
            i++;
            int candidateStart = i;
            bool candidateHasEscape = false;

            while (i < text.Length && text[i] != '"')
            {
                if (text[i] == '\\')
                {
                    candidateHasEscape = true;
                    i += 2;
                    continue;
                }
                i++;
            ...
            if (i >= text.Length)
            {
                return false; // unterminated string value — the file is malformed past this point
            }

            valueStart = candidateStart;
            valueEnd = i;
            hasEscape = candidateHasEscape;
            return true;
        }
    ...
    private JsonObject ParseObject(string json, string sourcePath)
    {
        // Total by contract (2026-08-21 lab-check follow-up to the M1 tolerant-bind fix): ANY
        // unusable file — malformed JSON syntax, or well-formed JSON containing DUPLICATE keys —
        // degrades THIS file to empty with a logged warning instead of escaping. Two live-proven
        // reasons the try/catch must sit here rather than around DeserializeTolerantly:
        //  1. JsonNode.Parse builds a LAZY JsonObject — duplicate property names don't throw at
        //     parse time; they throw ArgumentException ("An item with the same key has already
        //     been added") at the FIRST enumeration/clone LATER IN THE MERGE, which is neither a
        //     JsonException nor a NotSupportedException, so the tolerant-bind catch never saw it
        //     and the kiosk landed on the config-failed card anyway (live-proven with a duplicated
        //     "ReconnectSeconds" in camerawall.local.json). Forcing .Count materializes the
        //     dictionary HERE, inside the catch.
        //  2. A plain syntax error (a dropped brace) throws JsonException straight out of
        //     JsonNode.Parse — same brick, same "a wallboard never dies over a typo" rule.
        // Empty (non-null) matches the m4 non-object-root degradation exactly: this FILE is
        // ignored, every other config source still loads, and the warning names the file.
        try
        {
            var node = JsonNode.Parse(json, nodeOptions: null, documentOptions: ParseOptions);
            if (node is JsonObject obj)
            {
                _ = obj.Count; // force InitializeDictionary — surfaces duplicate keys right here
                return obj;
            }

            // m4 fix (2026-08-21 external audit): a non-object root (`[ ... ]`, a bare string/number)
            // used to silently become an empty config — the kiosk then showed the "not configured"
            // card with ZERO log trace explaining why a file full of settings was being ignored. Same
            // empty-config degradation (the file is still unusable), but named in the log now.
            _log?.Invoke(LogLevel.Warning,
                $"'{sourcePath}' does not contain a JSON object at its root (found {node?.GetValueKind().ToString() ?? "null"}) — " +
                "the file is ignored and defaults apply. The config file must start with '{' and end with '}'.");
            return new JsonObject();
        }
        catch (ArgumentException ex)
        {
            // Duplicate keys. Wholesale-ignoring the file (the malformed-syntax handling below)
            // proved too blunt in live testing: a deployment whose real settings live entirely in
            // an overlay holding ONE duplicated key went dark to the config-missing card. Salvage
            // instead with mainstream JSON-tool semantics — LAST occurrence of each name wins —
            // by rebuilding the object through JsonDocument (which tolerates duplicates) and let
            // the tolerant bind judge whatever values survive. Remaining known limit: duplicates
            // NESTED inside a subsection still reject at that subsection's own enumeration; the
            // top-level salvage covers the realistic operator case.
            try
            {
                using var doc = System.Text.Json.JsonDocument.Parse(json, ParseOptions);
                var salvaged = new JsonObject();
                foreach (var property in doc.RootElement.EnumerateObject())
                {
                    // JsonNode.Parse of the raw text, NOT JsonValue.Create: a JsonValue can only
                    // wrap SCALARS ("The element cannot be an object or array"), and a real
                    // config carries subsections (Health/WindowBounds) and arrays
                    // (RecordingServers/Monitors) — live-proven when exactly that threw
                    // InvalidOperationException out of the first salvage attempt.
                    salvaged[property.Name] = System.Text.Json.Nodes.JsonNode.Parse(property.Value.GetRawText());
                }

                _log?.Invoke(LogLevel.Warning,
                    $"'{sourcePath}' contains duplicate JSON keys ({ex.Message}) — keeping the LAST " +
                    "occurrence of each duplicated key and loading the file normally. Remove the " +
                    "duplicate to clear this warning.");
                return salvaged;
            }
            catch (Exception inner) when (inner is JsonException or ArgumentException)
            {
                _log?.Invoke(LogLevel.Warning,
                    $"'{sourcePath}' could not be parsed ({inner.GetType().Name}: {inner.Message}) — the file is " +
                    "ignored and defaults apply for its settings; every other config source still loads. " +
                    "Fix the file's JSON syntax to restore those settings.");
                return new JsonObject();
            }
        }
        ...
        {
            _log?.Invoke(LogLevel.Warning,
                $"'{sourcePath}' could not be parsed ({ex.GetType().Name}: {ex.Message}) — the file is " +
                "ignored and defaults apply for its settings; every other config source still loads. " +
                "Fix the file's JSON syntax to restore those settings.");
            return new JsonObject();
        }
