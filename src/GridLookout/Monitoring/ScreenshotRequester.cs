...
/// See docs/camerawall-admin-guide.md's "Remote screenshot" section for the operator-facing version
/// of that caveat.
///
...
public static class ScreenshotRequester
{
        ...
        TimeSpan? timeout = null)
    {
        ...
        try
        {
            ...
            // request's, or months-old imagery from a different account/deployment split) with exit
            // 0 whenever the current capture wrote nothing — misleading "success" in exactly the
            // failed-capture case a remote operator most needs to notice. Now only files provably
            // fresh from THIS request are printed with exit 0; a Done with no fresh files is its
            // own exit code, naming where to look.
            var freshFiles = ListFreshScreenshotFiles(candidateScreenshotDirectories, requestedUtc);
            if (freshFiles.Count == 0)
            {
                stdout.WriteLine(
                    "GridLookout answered, but no screenshot file from THIS request was found — the capture " +
                    "likely failed on the wall's side (locked/idle session, unwritable output directory). " +
                    "Check the wall's log for 'Screenshot capture failed'.");
                return ExitCodeNoFreshCapture;
            }

            foreach (var path in freshFiles)
            {
    ...
    /// <summary>Exit code 4 (m5 fix) — the responder signalled Done but no file provably written by
    /// THIS request exists in any candidate directory: the capture failed on the wall's side (see
    /// the message printed alongside). Replaces the old behavior of printing whatever stale files
    /// happened to exist with exit 0.</summary>
    public const int ExitCodeNoFreshCapture = 4;
    ...
    /// <c>screen-2.png</c>, not before it): the FIRST candidate whose newest file's write time is at
    /// or after <paramref name="requestedUtc"/> minus <see cref="FreshnessTolerance"/> — proof that
    /// file came from THIS request, not a leftover from a different account/deployment (see this
    /// class's own "FRESHNESS" doc section). m5 fix: the old any-file-at-all fallback is GONE — a
    /// stale-only result now returns empty, and <see cref="Run"/> reports it as
    /// <see cref="ExitCodeNoFreshCapture"/> instead of printing months-old imagery with exit 0.
    /// </summary>
    private static IReadOnlyList<string> ListFreshScreenshotFiles(IReadOnlyList<string> candidateDirectories, DateTime requestedUtc)
    {
