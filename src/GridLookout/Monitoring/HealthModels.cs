...
/// docs/camerawall-security.md's health.json section for the full content-class disclosure this
...
public sealed class WallHealthState
{
    ...
    /// <summary>M3 fix (2026-08-21 external audit): count of UI-thread exceptions
    /// (<c>Application.ThreadException</c>) this process has swallowed. WinForms routes UI-thread
    /// exceptions there — NOT to <c>AppDomain.UnhandledException</c> — so pre-fix they were logged
    /// and swallowed with zero health impact: an exception mid-rebuild could leave a half-torn wall
    /// whose pulse kept ticking, invisible to both the crash-relaunch guard and the probe. Product
    /// decision (owner, 2026-08-21): DEGRADE, never relaunch — a transient tick exception must not
    /// cost a wall restart, but health must stop claiming Healthy when the UI's integrity can no
    /// longer be assumed. Deliberately STICKY for the process's remaining life (no decay window):
    /// nothing can verify a half-torn wall healed itself, so the honest report is Degraded until an
    /// operator restarts at their convenience. See <see cref="HealthStatusCalculator.Compute"/>.</summary>
    public int UiThreadExceptionCount { get; set; }

    /// <summary>UTC timestamp of the most recent swallowed UI-thread exception — null until the
    /// first one. Lets a collector distinguish an old one-off from an active exception storm even
    /// though <see cref="UiThreadExceptionCount"/> alone already degrades health either way.</summary>
    public DateTime? LastUiThreadExceptionUtc { get; set; }
