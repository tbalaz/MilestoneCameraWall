...
/// <c>--health-probe</c> invocation only — see docs/camerawall-security.md for the full network
...
public sealed class HealthConfig
{
    ...
    /// carry recorder display names — see camerawall-security.md's health.json content-class
...
public sealed class WallConfig
{
    ...
    /// <summary>m2 fix (2026-08-21 external audit): per-day log byte cap, in megabytes — applied to
    /// <c>FileLogger.MaxMegabytesPerDay</c> by <c>Program.Main</c> alongside
    /// <see cref="LogRetentionDays"/>. When a day's log file reaches the cap, one final "cap
    /// reached" line is written and further lines that day are dropped and counted (a notice names
    /// the count on the next successful write). Bounds a runaway error storm on an unattended
    /// kiosk; day-count retention alone never bounded a SINGLE day's growth. Default <c>50</c>.
    /// <c>0</c> disables the cap.</summary>
    public int LogMaxMegabytesPerDay { get; set; } = 50;
