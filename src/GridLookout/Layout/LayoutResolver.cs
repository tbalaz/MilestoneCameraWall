...
/// that monitor's wall (see <c>camerawall-user-guide.md</c>'s "Layout tokens control everything when
...
public static class LayoutResolver
{
    ...
    {
        var catalogById = IndexCatalogById(input.Catalog);
        ...
        IReadOnlyDictionary<string, Guid> bindings)
    {
        ...
        var catalogById = IndexCatalogById(catalog);
    ...
    /// <summary>Nit fix (2026-08-21 external audit): both catalog indexers above were bare
    /// <c>ToDictionary(c =&gt; c.Id)</c> calls — a duplicate catalog id escaped as .NET's generic
    /// "An item with the same key has already been added" <see cref="ArgumentException"/>, which
    /// names nothing. The upstream pipeline dedupes (Milestone.RecorderCatalog's first-wins pass,
    /// warned), so a duplicate reaching this class means that pass was bypassed by a new catalog
    /// source; this guard makes the invariant explicit and the failure self-describing. Contained
    /// today by RebuildWall's try/catch, exactly as the pre-fix throw was.</summary>
    private static IReadOnlyDictionary<Guid, CameraCatalogEntry> IndexCatalogById(
        IReadOnlyList<CameraCatalogEntry> catalog)
    {
        var byId = new Dictionary<Guid, CameraCatalogEntry>(catalog.Count);
        foreach (var entry in catalog)
        {
            if (byId.ContainsKey(entry.Id))
            {
                throw new InvalidOperationException(
                    $"Camera catalog entry Id '{entry.Id}' is a duplicate — Milestone.RecorderCatalog's first-wins dedupe should have dropped it; refusing to index an ambiguous catalog.");
            }
