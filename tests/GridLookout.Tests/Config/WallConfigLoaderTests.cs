...
public class WallConfigLoaderTests : IDisposable
{
    ...
    private (WallConfigLoader Loader, List<string> Warnings) LoaderWithWarningCapture()
    {
        ...
        var loader = new WallConfigLoader(new FakeSecretProtector(),
            stateDirectory: new FakeStateDirectory(writable: false, Path.Combine(_dir, "state")),
            log: (level, msg) =>
        {
    ...
    // --- Live-proven 2026-08-21 lab check (M1 follow-up): two MORE brick doors the tolerant bind
    // didn't cover, because they explode before DeserializeTolerantly ever runs. Duplicate keys
    // salvage LAST-OCCURRENCE-WINS (wholesale-ignoring the file proved too blunt live: a
    // deployment whose settings lived entirely in one duplicated-key overlay went dark); malformed
    // syntax degrades THE FILE to empty-with-warning. Either way every other config source loads. ---

    [Fact]
    public void LoadOrCreate_DuplicateJsonKey_InOverlay_LastOccurrenceWins_PrimarySurvives()
    {
        // JsonNode.Parse builds a LAZY JsonObject — duplicate property names surface as
        // ArgumentException ("An item with the same key has already been added") at the first
        // enumeration during the merge, which no JsonException catch covered: live-proven to land
        // the kiosk on the config-failed card with a duplicated "ReconnectSeconds". The salvage
        // keeps the file's LAST value per key ("ten", itself mistyped -> tolerant bind drops it)
        // and everything else in the file still loads.
        var primaryPath = Path.Combine(_dir, "camerawall.json");
        WriteJson(primaryPath, """{ "ManagementServerUri": "http://vms-mgmt.example.local", "StaleSeconds": 20 }""");
        var overlayPath = Path.Combine(_dir, "camerawall.local.json");
        // Deliberately includes a subsection and an array: JsonValue.Create-based salvage could
        // only wrap scalars and threw InvalidOperationException on exactly these (live-proven).
        WriteJson(overlayPath, """{ "ShowHeader": true, "Health": { "Enabled": true }, "RecordingServers": [ { "HostName": "rec-a" } ], "ReconnectSeconds": 10, "ReconnectSeconds": "ten" }""");

        var (loader, warnings) = LoaderWithWarningCapture();
        var config = loader.LoadOrCreate(_dir, overlayFileName: "camerawall.local.json");

        Assert.Equal("http://vms-mgmt.example.local", config.ManagementServerUri); // primary survives
        Assert.Equal(20, config.StaleSeconds);
        Assert.True(config.ShowHeader); // salvaged from the overlay — only the duplicated key's fate changed
        Assert.True(config.Health.Enabled); // subsection survives salvage
        Assert.Single(config.RecordingServers); // array survives salvage
        Assert.Equal("rec-a", config.RecordingServers[0].HostName);
        Assert.Equal(15, config.ReconnectSeconds); // last occurrence was mistyped -> built-in default
        Assert.Contains(warnings, w => w.Contains("duplicate JSON keys"));
        Assert.Contains(warnings, w => w.Contains("ReconnectSeconds") && w.Contains("wrong type"));
    }

    [Fact]
    public void LoadOrCreate_MalformedJsonSyntax_InPrimary_DegradesToDefaultsWithWarning_NoThrow()
    {
        // A dropped brace is as much "a typo a wallboard never dies over" as a mistyped value —
        // pre-fix the JsonException escaped LoadOrCreate into the config-error card.
        var primaryPath = Path.Combine(_dir, "camerawall.json");
        WriteJson(primaryPath, """{ "ManagementServerUri": "http://vms-mgmt.example.local", "MaxFps": 12,,, }""");

        var (loader, warnings) = LoaderWithWarningCapture();
        var config = loader.LoadOrCreate(_dir, overlayFileName: null);

        Assert.Equal(string.Empty, config.ManagementServerUri); // file unusable -> defaults
        Assert.Equal(12, config.MaxFps); // built-in default
        Assert.Contains(warnings, w => w.Contains("camerawall.json") && w.Contains("could not be parsed"));
    }

    [Fact]
    public void LoadOrCreate_DuplicateJsonKey_InPrimary_LastOccurrenceWins()
    {
        var primaryPath = Path.Combine(_dir, "camerawall.json");
        WriteJson(primaryPath, """{ "Username": "svc-wall", "StaleSeconds": 15, "StaleSeconds": 45 }""");

        var (loader, warnings) = LoaderWithWarningCapture();
        var config = loader.LoadOrCreate(_dir, overlayFileName: null);

        Assert.Equal("svc-wall", config.Username);
        Assert.Equal(45, config.StaleSeconds); // LAST occurrence wins, mainstream JSON semantics
        Assert.Contains(warnings, w => w.Contains("duplicate JSON keys"));
    }
